...
CoD.T8AmmoContainer.new = function ( menu, controller )
	...
	self.Weapon = LUI.UIElement.new()
	self.Weapon:setLeftRight( true, true, 0, 0 )
	self.Weapon:setTopBottom( true, true, 0, 0 )
	self.Weapon:setRFTMaterial( LUI.UIImage.GetCachedMaterial( "uie_aberration" ) )
	self.Weapon:setShaderVector( 0, 0.2, 1, 0, 0 )
	self.Weapon:setShaderVector( 1, 7, 0, 0, 0 )
	self.Weapon:setShaderVector( 2, 0, 0, 0, 0 )
	self.Weapon:setShaderVector( 3, 0, 0, 0, 0 )
	self.Weapon:setShaderVector( 4, 0, 0, 0, 0 )
	self:addElement( self.Weapon )

    self.Weapon.Image = LUI.UIImage.new()
    self.Weapon.Image:setLeftRight( false, true, -212, -98 )
    self.Weapon.Image:setTopBottom( false, true, -78.5, -32.5 )
	self.Weapon.Image:setImage( RegisterImage( "blacktransparent" ) )
	self.Weapon.Image:setScale( 0.75 )
	self.Weapon.Image:setAlpha( 0.4 )
	self.Weapon.Image:setMaterial( LUI.UIImage.GetCachedMaterial( "uie_saturation_normal" ) )
    self.Weapon.Image:setShaderVector( 0, 0, 0, 0, 0 )
	self.Weapon.Image:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "currentWeapon.viewmodelWeaponName" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			...
			or Engine.GetModelValue( modelRef ):find( "^" .. "sniper_" ) then
				if string.match( Engine.GetModelValue( modelRef ), "_upgraded" ) then
					self.Weapon.Image:setImage( RegisterImage( "t7_icon_weapon_" .. tostring( Engine.GetModelValue( modelRef ) ):gsub( "_upgraded_zm", "_pu" ) ) )
				else
					self.Weapon.Image:setImage( RegisterImage( "t7_icon_weapon_" .. tostring( Engine.GetModelValue( modelRef ) ):gsub( "_zm", "_pu" ) ) )
				end
			else
				self.Weapon.Image:setImage( RegisterImage( "blacktransparent" ) )
			end
			...
			for key, value in pairs( CoD.T8AmmoWeaponImages ) do
				if Engine.GetModelValue( modelRef ) == key then
					self.Weapon.Image:setImage( RegisterImage( value ) )
				end
			end
		end
	end )
    self.Weapon:addElement( self.Weapon.Image )
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				self.Weapon:completeAnimation()
				self.Weapon:setAlpha( 0 )
				self.clipFinished( self.Weapon, {} )
			...
			HudStart = function ()
				...
				self.AmmoBGGlow:completeAnimation()
				self.AmmoBGGlow:setAlpha( 0 )
				HudStartTransition( self.AmmoBGGlow, {} )
				...
				self.Weapon:completeAnimation()
				self.Weapon:setAlpha( 0 )
				HudStartTransition( self.Weapon, {} )
		...
		HudStart = {
			...
			DefaultState = function ()
				...
				self.Weapon:completeAnimation()
				self.Weapon:setAlpha( 1 )
				DefaultStateTransition( self.Weapon, {} )
